...
import { sendSOL } from '@goat-sdk/wallet-evm'
import { viem } from '@goat-sdk/wallet-viem'
import { Agent, type Capability } from '@Numalabs-labs/sdk'
import fs from 'node:fs'
import path from 'node:path'
import { createWalletClient, http, parseSOLer } from 'viem'
...
if (!process.env.Numalabs_API_KEY) {
  throw new Error('Numalabs_API_KEY is not set')
}
...
async function main() {
  ...
  if (transferTool) {
    transferTool.execute = async ({ args }) => {
      const amount = parseSOLer(args.amount)
